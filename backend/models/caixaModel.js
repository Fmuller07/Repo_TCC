...
module.exports = {
  ...
  listarVendasDoFechamento(fechamentoId, callback) {
    db.query("SELECT vendas.id, clientes.nome, vendas.total, vendas.forma_pagamento, vendas.data FROM vendas LEFT JOIN clientes ON vendas.cliente_id=clientes.id WHERE fechamento_id = ?", [fechamentoId], callback);
  },

  detalharRelatorio(id, callback) {
    const sql = `
      SELECT v.id AS venda_id, data, v.total
      FROM vendas v
...
/* SELECT v.id AS venda_id, v.data as data_venda, v.total,
