  ...
  async registrar(req, res) {
    ...
    try {
      ...
      try {
        ...
        for (const [index, p] of produtos.entries()) {
          ...
          if (produto.estoque < p.quantidade) {
            throw new Error(
              ...
              }. Disponível: ${produto.estoque}, Solicitado: ${p.quantidade}`
            );
          }
        }

        // 2) Inserir venda
        const insertVenda = await queryConn(
          conn,
        ...
        if (
          forma_pagamento &&
          forma_pagamento.toString().trim().toLowerCase() === "fiado"
        ) {
          if (!cliente_id) {
          ...
          const clienteRows = await queryConn(
            conn,
